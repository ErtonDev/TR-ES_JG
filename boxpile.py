...
"}")
    ...
    def doRegister(self):
        ...
        if userInput == "" or passInput == "":
            ...
            if repeat == "f":
                popup = QMessageBox()
                popup.setWindowTitle("Nueva cuenta")
                popup.setText(f"Se va a crear la cuenta '{userInput}' con la contraseña '{passInput}.'")
                popup.setDetailedText("Los caracteres ilegales se eliminan automaticamente al crear una cuenta. (" + '"' + "," + " '" + ")")
    ...
    def addEvent(self):
        ...
        if event_name != "":
            ...
            if event_name in db_day:
                self.displayPopup(f"El nombre {event_name} ya pertenece a un evento existente del mismo día.")
                return
...
"}")
        ...
        if name != "":
            ...
            for i in db_classes:
                if name.lower() == i[0].lower():
                    self.displayPopup(f"El nombre {name} ya pertenece a una clase existente.")
                    return
    ...
    def addMark(self):
        ...
        try:
            ...
            self.displayPopup("Introduce un valor entre 0 y 10 para añadir un resultado a la clase y recalcular la media.")
            return
    ...
    def restartCourse(self):
        ...
        popup = QMessageBox()
        popup.setWindowTitle("Reiniciar Curso")
        popup.setText("Al pulsar Ok eliminará permanentemente todos los datos del curso académico actual.")
        popup.setIcon(QMessageBox.Warning)
        popup.setStandardButtons(QMessageBox.Close | QMessageBox.Ok)
        popup.setDefaultButton(QMessageBox.Close)
        result = popup.exec_()
...
"  Sesión"))
        ...
        if current_index == 1:
            ...
            if sharing != []:
                self.calendarioAddLabel2.setText(_translate("MainWindow", f"Compartiendo con {len(sharing)} ({sharing[0]}, ...)"))
            else:
        ...
        if current_index == 2:
            ...
            if db_classes[current_subindex][7] == "N/a No hay datos":
                self.seguimientoMainSubtitle.setText(_translate("MainWindow", "No hay datos"))
            else:
                self.seguimientoMainSubtitle.setText(_translate("MainWindow", f"Actualizado el {db_classes[current_subindex][7]}"))
            if db_classes[current_subindex][0] != "Global":
                self.seguimientoInMainTitle.setText(_translate("MainWindow", "Historial global de " + db_classes[current_subindex][0]))
            else:
            ...
            if self.seguimientoInMainPrev1.text() != "N/a No hay datos" and self.seguimientoInMainPrev2.text() != "N/a No hay datos":
            ...
            if self.seguimientoInMainPrev2.text() != "N/a No hay datos" and self.seguimientoInMainPrev3.text() != "N/a No hay datos":
            ...
            if self.seguimientoInMainPrev3.text() != "N/a No hay datos" and self.seguimientoInMainPrev4.text() != "N/a No hay datos":
        ...
        if current_index == 4:
            ...
            self.userMainCourseBtn.setText(_translate("MainWindow", "Reiniciar Curso"))
            if friends_num == 1:
                self.userMainFriendsLabel.setText(_translate("MainWindow", f"{friends_num} Amigo"))
            else:
        ...
        if current_index == 5:
            ...
            if friends_num == 1:
                self.accountMainFriendsLabel.setText(_translate("MainWindow", f"{friends_num} Amigo"))
            else:
