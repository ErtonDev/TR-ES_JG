...
# NOTE(Erton): Added one more slot in classes lists so before using doRegister() delete all current accounts
    ...
    def addEvent(self):
        ...
        if event_name != "":
            ...
            if event_name in db_day:
                self.displayPopup("No puedes crear dos eventos con el mismo nombre en un día. Prueba con otro nombre similar.")
                return
...
"  Sesión"))
        ...
        if current_index == 1:
            ...
            if sharing != []:
                self.calendarioAddLabel2.setText(_translate("MainWindow", f"Compartiendo con {len(sharing)}"))
            else:
